...
public class Iframe_Pages extends BaseClass {
	By demo1 = By.linkText("Demo Sites");
	By auto1 = By.linkText("Practice Automation");
	By iframe = By.linkText("Demo Site – iFrames");
	By tages = By.tagName("a");
	By pop = By.name("alertbox");
	...
	By main = By.cssSelector("button[class='navbar-toggle collapsed hidden-lg hidden-md']");
	By drop = By.xpath("//li[@id='menu-item-6372']//i[@class='fa fa-angle-right']");
	By register = By.xpath("//li[@id='menu-item-4134']//a[contains(text(),'Demo Site – Registration Form')]");

	public void naviagte_To_Iframes_Page() throws InterruptedException {
		...
		WebElement iframes = Xpath(iframe);
		wait.until(ExpectedConditions.visibilityOf(iframes));
		click(iframe);
		switchToIframeName("homepage");

		List<WebElement> urls = driver.findElements(tages);
...
//		}
		...
		switchToIframeName("popuppage");
		click(main);
		WebElement drop1 = Xpath(drop);
		wait.until(ExpectedConditions.visibilityOf(drop1));
		click(drop);
		driver.switchTo().defaultContent();
		

		switchToIframeName("formpage");
